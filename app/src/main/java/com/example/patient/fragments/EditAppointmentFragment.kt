...
class EditAppointmentFragment : Fragment(), View.OnClickListener, AdapterView.OnItemSelectedListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
		...
		model = AppointmentViewModel.getInstance(myContext)

		editAppointmentSpinner = root.findViewById(R.id.editAppointmentSpinner)

		allAppointmentappointmentIds = model.allAppointmentAppointmentIds()
	...
	private fun editAppointmentSearch() {
		...
		if (appointmentBean.isSearchAppointmentIdError(allAppointmentappointmentIds)) {
			...
			val selectedItem = model.searchByAppointmentappointmentId(appointmentIdData)
				appointmentIdTextField.setText(selectedItem[0].appointmentId.toString())
				codeTextField.setText(selectedItem[0].code.toString())
				Toast.makeText(myContext, "search Appointment is done!", Toast.LENGTH_LONG).show()
		}
	}
	...
	private fun editAppointmentOK() {
		...
		Toast.makeText(myContext, "Appointment edited!", Toast.LENGTH_LONG).show()
		}
	}
